...
test.skip("Test create run with revision id", async () => {
  ...
  try {
    ...
    expect(run1.extra?.metadata).not.toHaveProperty("LANGCHAIN_***REMOVED***");
    ...
    expect(run2.extra?.metadata).not.toHaveProperty("LANGCHAIN_***REMOVED***");
  } finally {
...
// This test requires OPENAI_***REMOVED*** to be set and valid
test("Test pull prompt include model", async () => {
  // eslint-disable-next-line no-process-env
  const openaiKey = process.env.OPENAI_***REMOVED***;
  if (!openaiKey || openaiKey.trim() === "" || openaiKey === "placeholder") {
    console.log("⚠️  Test skipped: OPENAI_***REMOVED*** not set or invalid");
    return;
  }
  ...
  try {
    ...
    if (error?.message?.includes("Missing secret")) {
      console.log(
        "⚠️  Test skipped: OPENAI_***REMOVED*** not valid for model loading"
      );
      return;
    }
    throw error;
