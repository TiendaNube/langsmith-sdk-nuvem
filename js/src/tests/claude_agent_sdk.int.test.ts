...
// Note: These tests require an ANTHROPIC_***REMOVED*** environment variable.
// They are skipped by default to avoid requiring API keys in CI.
//
// To run these tests:
// 1. Set ANTHROPIC_***REMOVED*** environment variable
...
describe("wrapClaudeAgentSDK - Real API Integration", () => {
  ...
  beforeAll(() => {
    if (!process.env.ANTHROPIC_***REMOVED***) {
      throw new Error("ANTHROPIC_***REMOVED*** environment variable is required");
    }
  });
