...
_OPENAI_***REMOVED*** = "OPENAI_***REMOVED***"
_ANTHROPIC_***REMOVED*** = "ANTHROPIC_***REMOVED***"
...
class Client:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            api_key (Optional[str]): API key for the LangSmith API. Defaults to the `LANGCHAIN_***REMOVED***`
    ...
    def _compute_headers(self) -> dict[str, str]:
        ...
        if self.api_key:
            headers[X_***REMOVED***] = self.api_key
    ...
    def _create_run_non_batch(
        ...
        *,
        ...
        if api_key is not None or api_url is not None:
            ...
            headers = {**self._headers, X_***REMOVED***: target_api_key}
            try:
            ...
            for write_api_url, write_api_key in self._write_api_urls.items():
                headers = {**self._headers, X_***REMOVED***: write_api_key}
                try:
                    self.request_with_retries(
                        "POST",
    ...
    def pull_prompt(
        ...
        *,
        ...
        !!! warning "Behavior changed in `langsmith` 0.5.1"
            ...
            have an OPENAI_***REMOVED***. Previously this was read from environment
            variables by default. To do so now you must specify
            `secrets={"OPENAI_***REMOVED***": "sk-..."}` or `secrets_from_env=True`.
        ...
        *,
        ...
        Example:
            ...
            report = client.generate_insights(
                ...
                openai_api_key=os.environ["OPENAI_***REMOVED***"],
            )
    ...
    def _ensure_insights_api_key(
        ...
        *,
        ...
        if model in (None, "openai"):
            target_keys.add(_OPENAI_***REMOVED***)
        if model in (None, "anthropic"):
            target_keys.add(_ANTHROPIC_***REMOVED***)

        if existing_keys := workspace_keys.intersection(target_keys):
            return "openai" if _OPENAI_***REMOVED*** in existing_keys else "anthropic"
        elif model == "openai":
            api_key = openai_api_key
            api_var = _OPENAI_***REMOVED***
        elif model == "anthropic":
            api_key = anthropic_api_key
            api_var = _ANTHROPIC_***REMOVED***
        elif openai_api_key or anthropic_api_key:
            api_key = openai_api_key or anthropic_api_key
            api_var = _OPENAI_***REMOVED*** if openai_api_key else _ANTHROPIC_***REMOVED***
        else:
        ...
        ls_utils.raise_for_status_with_text(response)
        return "openai" if api_var == _OPENAI_***REMOVED*** else "anthropic"
