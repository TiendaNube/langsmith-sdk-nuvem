...
def test_validate_api_key(monkeypatch: pytest.MonkeyPatch) -> None:
    # Scenario 1: Both LANGCHAIN_***REMOVED*** and LANGSMITH_***REMOVED*** are set,
    # but api_key is not
    _clear_env_cache()
    monkeypatch.setenv("LANGCHAIN_***REMOVED***", "env_langchain_api_key")
    monkeypatch.setenv("LANGSMITH_***REMOVED***", "env_langsmith_api_key")

    client = Client()
    assert client.api_key == "env_langsmith_api_key"

    # Scenario 2: Both LANGCHAIN_***REMOVED*** and LANGSMITH_***REMOVED*** are set,
    # and api_key is set
    _clear_env_cache()
    monkeypatch.setenv("LANGCHAIN_***REMOVED***", "env_langchain_api_key")
    monkeypatch.setenv("LANGSMITH_***REMOVED***", "env_langsmith_api_key")
    ...
    # Scenario 3: LANGCHAIN_***REMOVED*** is set, but LANGSMITH_***REMOVED*** is not
    monkeypatch.setenv("LANGCHAIN_***REMOVED***", "env_langchain_api_key")
    monkeypatch.delenv("LANGSMITH_***REMOVED***", raising=False)

    client = Client()
    assert client.api_key == "env_langchain_api_key"

    # Scenario 4: LANGCHAIN_***REMOVED*** is not set, but LANGSMITH_***REMOVED*** is set
    _clear_env_cache()
    monkeypatch.delenv("LANGCHAIN_***REMOVED***", raising=False)
    monkeypatch.setenv("LANGSMITH_***REMOVED***", "env_langsmith_api_key")
...
def test_create_run_includes_langchain_env_var_metadata(
    ...
    with patch.dict("os.environ", {"LANGCHAIN_REVISION": "abcd2234"}):
        ...
        if auto_batch_tracing:
            ...
            assert "LANGCHAIN_***REMOVED***" not in posted_value["extra"]["metadata"]
...
def test_host_url(_: MagicMock) -> None:
    client = Client(api_url="https://api.foobar.com/api", api_key="***REMOVED***")
    ...
    client = Client(api_url="http://localhost:8000", api_key="***REMOVED***")
    assert client._host_url == "http://localhost"

    client = Client(api_url="https://eu.api.smith.langchain.com", api_key="***REMOVED***")
    assert client._host_url == "https://eu.smith.langchain.com"

    client = Client(api_url="https://dev.api.smith.langchain.com", api_key="***REMOVED***")
    assert client._host_url == "https://dev.smith.langchain.com"

    client = Client(api_url="https://api.smith.langchain.com", api_key="***REMOVED***")
...
@mock.patch.dict(
    os.environ,
    {"LANGCHAIN_***REMOVED***": "", "LANGSMITH_***REMOVED***": "", "LANGSMITH_TRACING": "true"},
    clear=True,
)
...
@mock.patch.dict(
    os.environ,
    {"LANGCHAIN_***REMOVED***": "", "LANGSMITH_***REMOVED***": "", "LANGSMITH_TRACING": "false"},
    clear=True,
)
...
def test_pull_prompt(
    ...
    with mock.patch.dict(
        "os.environ",
        {
            "ANTHROPIC_***REMOVED***": "test_anthropic_key",
            "OPENAI_***REMOVED***": "test_openai_key",
        },
    ):
    ...
    with (
        ...
        with mock.patch.dict(
            "os.environ",
            {
                "ANTHROPIC_***REMOVED***": "test_anthropic_key",
                "OPENAI_***REMOVED***": "test_openai_key",
                "LANGSMITH_***REMOVED***": "fake_api_key",
...
def test_workspace_headers_injection(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # env setup
    monkeypatch.setenv("LANGSMITH_***REMOVED***", "test-key")
...
class TestEndToEndWorkspaceFlow:
    ...
    def test_successful_api_call_with_workspace_id(
        ...
        # env setup
        monkeypatch.setenv("LANGSMITH_***REMOVED***", "test-api-key")
        monkeypatch.setenv("LANGSMITH_WORKSPACE_ID", "test-workspace-id")
